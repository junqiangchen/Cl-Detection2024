...
class VNet2d(nn.Module):
    ...
    def __init__(self, image_channel, numclass, init_features=16, drop=0.2):
        super(VNet2d, self).__init__()
        self.image_channel = image_channel
        self.numclass = numclass
        self.features = init_features

        self.in_tr = InputTransition2d(self.image_channel, self.features, drop)

        self.down_tr32 = DownTransition2d(self.features, self.features * 2, 2, drop)
        self.down_tr64 = DownTransition2d(self.features * 2, self.features * 4, 3, drop)
        self.down_tr128 = DownTransition2d(self.features * 4, self.features * 8, 3, drop)
        self.down_tr256 = DownTransition2d(self.features * 8, self.features * 16, 3, drop)

        self.up_tr256 = UpTransition2d(self.features * 16, self.features * 8, 3, drop)
        self.up_tr128 = UpTransition2d(self.features * 8, self.features * 4, 3, drop)
        self.up_tr64 = UpTransition2d(self.features * 4, self.features * 2, 2, drop)
        self.up_tr32 = UpTransition2d(self.features * 2, self.features, 1, drop)
