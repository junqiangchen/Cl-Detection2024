...
from .dataset import datasetModelSegwithopencv, datasetModelSegwithnpy, datasetModelSegwithnpymutil, \
    datasetModelRegressionwithnpy, datasetModellandmarkwithnpy
from torch.utils.data import DataLoader
from .losses import BinaryDiceLoss, BinaryFocalLoss, BinaryCrossEntropyLoss, BinaryCrossEntropyDiceLoss, \
    MutilDiceLoss, MutilFocalLoss, MutilCrossEntropyLoss, MutilCrossEntropyDiceLoss, MSELoss, L1Loss, L2Loss, \
    Focal_Loss_Regression
import torch.optim as optim
from torch.cuda.amp import autocast as autocast
from torch.cuda.amp import GradScaler
import numpy as np
from tqdm import tqdm
from .metric import dice_coeff, iou_coeff, multiclass_dice_coeff, multiclass_iou_coeff, calc_ssim, calc_psnr
...
class VNet3dRegressionModel(object):
    ...
    def __init__(self, image_depth, image_height, image_width, image_channel, numclass, batch_size,
                 loss_name='MSE', inference=False, model_path=None, amp=False, accum_iter=4, num_cpu=8, use_cuda=True):
        self.batch_size = batch_size
        self.loss_name = loss_name
...
class VNet2dlandmarkdetectionModel(object):
    """
    Vnet2d with binary class,should rewrite the dataset class and inference fucntion
    """

    def __init__(self, image_height, image_width, image_channel, numclass, batch_size, loss_name='L1',
                 inference=False, model_path=None, amp=False, accum_gradient_iter=1, num_cpu=8, use_cuda=True):
        self.batch_size = batch_size
        self.loss_name = loss_name
        self.accuracyname = 'EuclideanDistance'
        self.image_height = image_height
        self.image_width = image_width
        self.image_channel = image_channel
        self.numclass = numclass
        ...
        self.model = VNet2d(self.image_channel, self.numclass, 32, 0)
        self.model.to(device=self.device)
    ...
    def _dataloder(self, images, labels, batch_size, shuffle=False):
        ...
        train dataset shuffle is true,validation is false
        """
        dataset = datasetModellandmarkwithnpy(images, labels)
        # fow window num_workers is only zero,for linux num_workers can not zero
        dataloader = DataLoader(dataset, shuffle=shuffle, batch_size=batch_size, num_workers=self.num_cpu,
                                pin_memory=True)
        return dataloader
    ...
    def _loss_function(self, lossname):
        ...
        if lossname == "focal_loss":
            return Focal_Loss_Regression()

    def __get_ave_xy(self, hmi, n_points=4, thresh=0):
        '''
        ...
        '''
        if n_points < 1:  ## Use all
            hsum, n_points = np.sum(hmi), len(hmi.flatten())
            ind_hmi = np.array([range(self.image_width)] * self.image_height)
            i1 = np.sum(ind_hmi * hmi) / hsum
            ind_hmi = np.array([range(self.image_height)] * self.image_width).T
    ...
    def __get_landmark(self, image):
        ...
        # coord = np.array(np.unravel_index(max_index, shape=image.shape), np.int)
        yy, xx = np.where(image == np.max(image))
        # there may be multiple maximum positions, and a simple average is performed as the final result
        x0, y0 = np.mean(xx), np.mean(yy)
        coord_ = [y0, x0]
        coord = [round(y0), round(x0)]
        value = image[tuple(coord)]
        return coord_, value

    def __get_landmarks(self, predictiamge, max=True):
        coords = []
        values = []
        for image in np.rollaxis(predictiamge, axis=0):
            if max:
                coord, value = self.__get_landmark(image)
                coords.append(coord)
                values.append(value)
            else:
                coord = self.__get_ave_xy(image, 5)
                coords.append(coord)
                values.append(1.0)
    ...
    def _accuracy_function(self, metric_name, Y_pred, Y_gt):
        Y_pred = Y_pred.detach().cpu().squeeze().numpy()
        Y_gt = Y_gt.detach().cpu().squeeze().numpy()
        ...
        if metric_name == "EuclideanDistance":
            if len(Y_pred.shape) != 3:
                ...
                pd_coords, pd_values = self.__get_landmarks(Y_pred)
                gt_coords, gt_values = self.__get_landmarks(Y_gt)
                distance = pd_coords - gt_coords
                distance_vector = distance.flatten()
                mertic = mertic + np.linalg.norm(distance_vector)
                return mertic
    ...
    def _train_loop(self, train_loader, totalTrainLoss, totalTrainAccu, trainshow, e):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if trainshow and e % self.showstep == 0:
                # save_images
                savepath = self.model_dir + '/' + str(e + 1) + "_Train_EPOCH_"
                save_images3d(pred_logit[0], y[0], self.showwind, savepath, pixelvalue=self.showpixelvalue)
                trainshow = False
            # add the loss to the total training loss so far
            totalTrainLoss.append(loss.cpu().detach().numpy() * self.accum_iter)
    ...
    def trainprocess(self, trainimage, trainmask, validationimage, validationmask, model_dir, epochs=50, lr=1e-3,
        ...
        self.showpixelvalue = 255.
        self.showwind = showwind
        self.showstep = epochs // self.shownumber
        # 1、initialize loss function and optimizer
        self.model.apply(initialize_weights)
        self.lossFunc = self._loss_function(self.loss_name)
        self.opt = optim.AdamW(self.model.parameters(), lr=lr)
        self.scheduler = optim.lr_scheduler.StepLR(self.opt, step_size=20, gamma=0.9)
        self.scaler = GradScaler()
        # 2、load data train and validation dataset
        train_loader = self._dataloder(trainimage, trainmask, self.batch_size, True)
    ...
    def get_landmarks(self, heatmaps_array, max=True):
        coords_array_pd, values_array_pd = self.__get_landmarks(heatmaps_array, max)
        return coords_array_pd, values_array_pd

    def inference(self, image):
        # process resize image and normalization
        imageresize = cv2.resize(image, (self.image_width, self.image_height), interpolation=cv2.INTER_LINEAR)
        imageresize = (imageresize - imageresize.mean()) / imageresize.std()
        offsety, offsetx = self.image_width / image.shape[0], self.image_height / image.shape[1]
        # transpose (H,W,C) order to (C,H,W) order
        H, W = np.shape(imageresize)[0], np.shape(imageresize)[1]
        imageresize = np.reshape(imageresize, (H, W, 1))
        imageresize = np.transpose(imageresize, (2, 0, 1))
        heatmaps_array = self.predict(imageresize)
        # resize heatmaps get coords and values
        coords_array_pd, values_array_pd = self.__get_landmarks(heatmaps_array)
        # 4 resize landmarks_coords to src image size
        resize_coords_array_pd = np.around(coords_array_pd * np.array((1 / offsety, 1 / offsetx))).astype('int')
        # max_heat_pd = np.sum(heatmaps_array, axis=0)
        # max_heat_pd = cv2.resize(max_heat_pd, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_LINEAR)
        max_heat_pd = np.zeros_like(image)
        for i in range(resize_coords_array_pd.shape[0]):
            cv2.circle(max_heat_pd, (resize_coords_array_pd[i][1], resize_coords_array_pd[i][0]), 3, 255, -1)
        return resize_coords_array_pd, max_heat_pd

    def inferencevalid(self, image, mask):
        imageresize = (image - image.mean()) / image.std()
        # transpose (H,W,C) order to (C,H,W) order
        H, W = np.shape(imageresize)[0], np.shape(imageresize)[1]
        imageresize = np.reshape(imageresize, (H, W, 1))
        imageresize = np.transpose(imageresize, (2, 0, 1))
        ...
        # resize heatmaps get coords and values
        coords_array_pd, values_array_pd = self.__get_landmarks(heatmaps_array)
        coords_array_gt, values_array_gt = self.__get_landmarks(label)
        # 4 resize landmarks_coords to src image size
        resize_coords_array_pd = np.around(coords_array_pd).astype('int')
        resize_coords_array_gt = np.around(coords_array_gt).astype('int')
        # 5 save key points result
        max_heat_pd = np.zeros_like(image)
        max_heat_gt = np.zeros_like(image)
        for i in range(resize_coords_array_pd.shape[0]):
            cv2.circle(max_heat_pd, (resize_coords_array_pd[i][1], resize_coords_array_pd[i][0]), 3, 255, -1)
        for i in range(resize_coords_array_gt.shape[0]):
            cv2.circle(max_heat_gt, (resize_coords_array_gt[i][1], resize_coords_array_gt[i][0]), 3, 255, -1)
        # max_heat_gt = np.sum(label, axis=0)
        # max_heat_pd = np.sum(heatmaps_array, axis=0)
        return resize_coords_array_pd, resize_coords_array_gt, max_heat_pd, max_heat_gt
