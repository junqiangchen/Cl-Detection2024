...
# define datasetModellandmarkwithnpy class wiht npy
class datasetModellandmarkwithnpy(Dataset):
    def __init__(self, images, labels):
        super(datasetModellandmarkwithnpy).__init__()
...
class datasetModelSegwithnpymutil(Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        imagepath = self.images[index]
        imagect = np.load(imagepath + '/image.npy')
        imagepet = np.load(imagepath + '/wall.npy')
        # transpose (D,H,W,C) order to (C,D,H,W) order
        D, H, W = np.shape(imagect)[0], np.shape(imagect)[1], np.shape(imagect)[2]
        image = np.zeros((self.targetsize[0], D, H, W))
        image[0, ...] = imagect[:]
        image[1, ...] = imagepet[:]
        assert image.shape[0] == self.targetsize[0] and image.shape[1] == self.targetsize[1] and image.shape[2] == \
               self.targetsize[2] and image.shape[3] == self.targetsize[3]
